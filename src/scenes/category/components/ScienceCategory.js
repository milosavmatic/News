...
class ScienceCategory extends React.Component {
    componentDidMount() {
        const {loadScienceCategory} = this.props
        loadScienceCategory()
    }
    ...
    render() {
        const { scienceCategory, navigation } = this.props
        ...
        return (
            <View style={styles.container}>
                <TouchableOpacity style={{flex: 1, alignItems: 'center'}} onPress={() => navigation.navigate('Category Container', {
                    ...
                    style={{fontSize: 25, fontWeight: 'bold'}}>{'Science'}</Text></TouchableOpacity>
                <Carousel data={scienceCategory.slice(0, 5)} renderItem={this.renderItem} sliderWidth={Dimensions.get('window').width}
